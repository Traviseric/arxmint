...
/** Generate a .env.template file for a community deployment */
export function generateEnvTemplate(config: CommunityConfig): string {
  const useCDK = config.memberCount > 30 || config.network === "bitcoin";
  const lines: string[] = [
    `# ArxMint — ${config.name}`,
    ...
    `GRAFANA_PASSWORD=changeme-$(openssl rand -hex 8)`,
    ``,
  ];

  if (useCDK) {
    lines.push(
      `# Cashu mint database credentials`,
      `CASHU_DB_USER=cashu`,
      `CASHU_DB_PASSWORD=changeme-$(openssl rand -hex 16)`,
      ``
    );
  } else {
    lines.push(
      `# Cashu mint private key (auto-generated if not set)`,
      `CASHU_PRIVATE_KEY=`,
      ``
    );
  }
  ...
  if (config.network === "bitcoin") {
    lines.push(
      `# ============================================================`,
      `# MAINNET WARNING — real sats, double-check everything`,
      `# ============================================================`,
      ``
    );
  }
  ...
  lines.push(
    `# ============================================================`,
    `# OPTIONAL — set for additional features`,
    `# ============================================================`,
    ``
  );
  ...
  if (config.agents.enabled) {
    lines.push(
      `# L402 / Aperture — macaroon for agent auth (generated by LND on first run)`,
      `LND_MACAROON_PATH=/root/.lnd/data/chain/bitcoin/${config.network === "bitcoin" ? "mainnet" : config.network}/admin.macaroon`,
      ``
    );
  }
  ...
  lines.push(
    ...
    `CLOUDFLARE_ZONE_ID=`,
    ``,
    `# ArxMint hosted API (optional — for saving config to dashboard)`,
    `ARXMINT_API_URL=https://arxmint.com`,
    `ARXMINT_MERCHANT_ID=`,
    ``
  );
  ...
  if (config.mintBackend === "fedimint") {
    lines.push(
      `# Fedimint federation — set after DKG ceremony`,
