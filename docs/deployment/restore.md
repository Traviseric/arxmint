# ArxMint Disaster Recovery â€” Restore Runbook
...
| Hardware failure / full data loss | `scripts/restore.sh` â† **this guide** |
...
# Edit .env.local â€” fill in secrets (CASHU_PRIVATE_KEY, NEXTAUTH_SECRET, etc.)
...
4. Wait for chain sync and channel recovery (can take 10â€“60 minutes on testnet)
...
### Step 5 â€” Restore Postgres Database
...
1. **Lightning balance:** `docker exec sf-lnd-lite lncli walletbalance` â€” should show your on-chain balance
2. **Channels:** `docker exec sf-lnd-lite lncli listchannels` â€” channels recover via SCB within ~10 minutes
3. **Cashu mint:** Open http://localhost:3338 â€” mint should respond
...
- `docs/operations/dr-drill.md` â€” quarterly drill checklist
- `scripts/backup.sh` â€” how backups are created
- `scripts/rollback.sh` â€” for bad update rollback (not hardware failure)
