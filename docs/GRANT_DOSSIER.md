...
**Budget ranges:** $75K (minimum viable) · $200K (full build-out)

---

## 1. Executive Summary

ArxMint is open-source infrastructure that lets anyone create a private Bitcoin circular economy from a single natural-language prompt. Type a description of your community — ArxMint generates Fedimint federation configs, Cashu ecash mint settings, Lightning L402 commerce rails, and privacy defaults — all deployable with one Docker command.

**The problem it solves:** Bitcoin communities that want to run local circular economies face three hard blockers. First, setting up Fedimint, Cashu, and Lightning together requires deep multi-protocol expertise that most community builders lack. Second, existing custodial wallets expose users to deplatforming risk and strip away financial privacy. Third, AI agents — the fastest-growing participants in digital commerce — have no shared private payment infrastructure with the humans they work alongside.
...
**Proof of work:** The Longmont, CO Bitcoin meetup is the first pilot target — 30 merchant target, 300 monthly active spenders within 6 months, 98%+ payment success rate. All code is MIT-licensed and live on GitHub today.
...
|-------|-----------|------|
| Frontend | Next.js 15, React 19, TypeScript | Prompt → config UI, wallet panel, merchant onboarding |
...
**Security boundaries:**
- Cashu proofs stored client-side only in AES-256-GCM encrypted IndexedDB vault — never in the database
...
**Production gate items** (Phases A–E) are in active development. The project has a clear Production Readiness Gate with explicit checkboxes across data safety, auth, payment correctness, infrastructure, and testing — no real funds accepted until all pass.

### What the Grant Funds

Grant funding accelerates three areas:

1. **Production hardening** — Phases A–E (database, vault, payments, infrastructure, testing). Currently built on mock data; grant funds wiring to real Bitcoin infrastructure.
2. **Pilot deployment** — Longmont, CO: server costs, merchant onboarding, NFC cards, community events, 6-month operation.
3. **Replication playbook** — Open-source documentation so any community can deploy their own ArxMint from the playbook.

---

## 3. Budget Template

### $75K Scenario — Minimum Viable Pilot

Focus: one pilot city (Longmont), production hardening, and replication playbook.

| Category | Description | Amount |
|----------|-------------|--------|
| Infrastructure | Vultr 16GB/6-core VPS ($80/mo × 12), backup storage, domain, SSL, initial Lightning channel funding (1M+ sats) | $6,000 |
| Development | Travis: Phase A–E completion, pilot deployment (approx. 600 hrs @ $100/hr effective) | $40,000 |
...
- Q1 (Months 1–3): 50% — Infrastructure + Phase A–E completion + first merchant cohort
- Q2 (Months 4–6): 30% — Growth phase, full merchant target, community events
- Q3 (Months 7–8): 20% — Evaluation, replication playbook, grant reporting

### $200K Scenario — Full Build-Out
...
| Development | Travis + 1 contributor: Phase A–E + Spire + Aether + Payment SDK (approx. 1,600 hrs total) | $120,000 |
...
**Milestone spend breakdown:**
- Q1: 30% — Production hardening + Longmont soft launch
- Q2: 25% — Longmont full launch + City 2 pre-launch
- Q3: 25% — City 2 launch + City 3 pre-launch, Spire features
- Q4: 20% — City 3 launch, Aether features, evaluation, replication playbook

---

## 4. Team Bios Template

### Travis [Last Name] — Lead Developer & Project Founder

_[Fill in: 2–3 sentences on Bitcoin/Lightning background. Example structure below.]_
...
- ArxMint (this project) — full-stack, all phases
- [Other OSS contributions — link to GitHub PRs or repos]
- [Community work — Longmont meetup, BTCPay/Fedimint deployments, etc.]
...
- **Non-custodial:** No server ever holds user funds. Cashu proofs live in the user's encrypted local vault (IndexedDB, AES-256-GCM). Server stores only transaction metadata — never proof secrets.
...
| User payment metadata | Postgres — amount, backend, status only | No proof secrets in DB |
...
These caps are not permanent — they are pilot-phase safeguards while the system is validated on testnet and then mainnet with real but bounded funds.
...
- **Why we do it anyway:** The engineering pilot validates the full software stack — community config generation, Docker deployment, merchant onboarding, L402 + NUT-24 payment flows, monitoring — before distributing guardians to multiple operators.
...
- **Mainnet migration plan:** Detailed in `docs/mainnet-migration-plan.md` (see task 144). Short version: freeze pilot, distribute guardians, re-DKG, reopen with production trust assumptions and appropriate value caps removed.
...
ArxMint's privacy dashboard shows per-backend support status honestly. Silent Payments for Fedimint peg-outs requires a federation-level module change — it is not a client toggle, and the dashboard labels it accordingly (Phase 0.2 Fortify fix).

---

*This dossier was generated from codebase artifacts by the ArxMint overnight agent pipeline on 2026-02-28. All figures drawn from `lib/pilot-deployment.ts` (KPI targets), `lib/grant-templates.ts` (narratives), `docs/roadmap.md` (phases), and `docs/spec.md` (architecture). Update this file when pilot data, team bios, or budget figures change.*
