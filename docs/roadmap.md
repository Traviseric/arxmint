...
| **Phase 5 custody model** | **Strictly non-custodial.** Self-hosted BTCPay Server model. ArxMint = open-source software only. Merchants run their own nodes. Payments are peer-to-peer. ArxMint never touches funds. Hosted model would require federal MSB registration + multi-state licensing + EU MiCA CASP + UK FCA authorization. | `docs/research/Phase5-Bazaar/1-Crypto Payment Infrastructure Legal Analysis.md` |
...
SELF-HOSTED MODEL (legally protected):
  ...
  ArxMint (the project) = open-source software only, zero custody
```
...
**What:** Replace the old "settlement automation" item with the critical self-hosted UX: a single Docker command that stands up a complete merchant payment node with LND, Cashu mint, checkout page, webhook engine, dashboard, and LNURL-pay — all pre-configured.
...
**Implementation:**
- `arxmint merchant init` — interactive CLI: business name, domain, payment methods (Lightning/Cashu/both)
- Generates: `docker-compose.merchant.yml` with LND + Cashu mint (Nutshell or CDK) + ArxMint merchant API + Caddy (auto-HTTPS)
- `docker compose -f docker-compose.merchant.yml up -d` — single command, merchant is live
- Auto-configures: LNURL-pay, checkout page routes, webhook engine, dashboard, local auth tokens
...
- Minimum spec: 2GB RAM, 1 vCPU, 20GB disk ($6-12/month VPS)
...
| 5.7 Merchant dashboard (self-hosted) | Planned | 5.1 + 5.4 + Phase A DB |
| 5.8 One-command merchant deploy | Planned | Phase C Docker stack + Caddy |
...
```
5.8 One-Command Deploy ──→ 5.1 Local Auth ──→ 5.4 Status API ──→ 5.2 Webhooks
                                │                                    │
...
|---|---|---|
| Coffee shop (no code) | `arxmint merchant init` + print QR | 10 minutes |
