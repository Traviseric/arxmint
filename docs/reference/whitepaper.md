...
Simultaneously, AI agents are becoming economic actors â€” buying data, selling compute, paying for API access â€” but every existing payment system requires human identity. Credit cards, bank accounts, and payment processors all assume a human at the other end. There is no native payment rail for machines.
...
This paper describes the architecture, privacy model, agent commerce protocol, economic incentives, security analysis, and deployment plan for ArxMint â€” including a six-month pilot targeting 30 merchants and 300 monthly active users in the Boulder-Longmont corridor of Colorado.
...
2. **Surveillance by default.** Every exchange, every on-ramp, and every compliant service builds a map of the user's financial life. Privacy is opt-in and expensive â€” the opposite of what electronic cash should be.
...
ArxMint exists to close this gap. It generates the complete infrastructure stack required for a community to transact in bitcoin privately â€” and it does so from a single natural language prompt.
...
Each component â€” Fedimint, Cashu, Lightning, privacy tools â€” exists as a separate project with its own documentation, Docker configuration, and operational requirements. Stitching them into a working economy takes weeks of DevOps expertise. Most communities never start.

### 2.2 The Agent Problem

AI agents are economic actors. They perform real work â€” analyzing data, monitoring markets, running computations â€” and need to be compensated. But every traditional payment system has a human gatekeeper:
...
AI agents are not humans. They do not have IDs, bank accounts, or business registrations. They need payment rails that work without identity â€” rails that already exist in the Bitcoin protocol but lack the integration layer to be useful.
...
This convergence is not incidental â€” it is the primary design constraint. Every architectural decision flows from the requirement that a single deployment serves both human and agent commerce.
...
Natural Language Prompt
  â†’ parsePrompt()         // Extract community parameters
  â†’ generateConfig()      // Build CommunityConfig
  â†’ generateDockerCompose()  // Multi-service YAML
  â†’ generateApertureConfig() // L402 proxy routes
  â†’ docker compose up -d     // Deploy
  â†’ Live sovereign economy
```
...
ArxMint creates a closed economic loop â€” the "Sovereign Loop" â€” where sats circulate without touching fiat:

1. **Earn** â€” Community members earn sats for real work: workshops, freelance services, merchant sales.
2. **Spend** â€” Private ecash payments at local merchants. Scan a QR, pay in sats.
3. **Agent Revenue** â€” AI agents earn sats by selling data, compute, and analytics through L402 paywalls, 24/7.
4. **Reinvest** â€” Agent earnings flow back into the local economy as the operator spends them at the same merchants.
...
- **Guardian scaling**: Guardian count scales with community size â€” `ceil(memberCount / 10)`, clamped to [3, 7]. Minimum 3 ensures Byzantine fault tolerance with f=1.
...
- **Multi-mint (Coco)**: `MultiMintManager` tracks multiple Cashu clients. Cross-mint transfers use Lightning as the atomic bridge â€” source mint melts to Lightning invoice, destination mint mints from Lightning payment.
...
| â‰¤ 30 members, testnet | Nutshell (Cashu) | Lightweight, minimal infrastructure |
| Explicit "federation" in prompt | Fedimint | User preference override |

### 4.4 Agent Wallets

Both Fedimint and Cashu support ephemeral agent wallets â€” in-memory-only wallets designed for autonomous processes:
...
Reissuance performs a proof swap â€” the mint receives old proofs and issues new ones that are cryptographically unlinkable from the originals. The audit log records that reissuance happened at a specific state, resolving the tension between ecash's privacy (unlinkable tokens) and accountability (provable spending history).
...
NUT-24 extends the same 402 pattern to Cashu ecash tokens. ArxMint implements dual paywall support â€” a single endpoint accepts both Lightning (L402) and ecash (NUT-24) payments:
...
This dual support is critical because it allows community members who hold ecash to access the same services as agents who hold Lightning balances â€” without requiring either party to convert.
...
3. Call `checkProofsStates()` â€” all proofs must be unspent
4. Call `receive()` â€” atomic proof swap prevents double-spend
...
This bridge collapses the distinction between "ecash user" and "Lightning user" â€” anyone holding ecash can access any L402-gated service.
...
Privacy in ArxMint is a default, not an option. The architecture assumes that financial transactions should be private unless the user explicitly chooses otherwise. This is not about evasion â€” it is about building infrastructure that makes Bitcoin work as money.
...
The spend router automatically selects the optimal payment path based on amount, available balances, and privacy preference. Users do not need to understand the underlying protocols â€” they specify an amount and optionally a privacy preference, and the router handles path selection.
...
| 10,000 â€“ 100,000 sats | Lightning | Ecash | Fast routing, good privacy for medium amounts |
| 100,000 â€“ 1,000,000 sats | Ark VTXOs | Lightning | Batch settlement privacy for large transfers |
| â‰¥ 1,000,000 sats | Silent Payments (on-chain) | Standard on-chain | Stealth addresses for large on-chain amounts |
...
| Lightning | ~0.1% (max(1, ceil(amount Ã— 0.001))) |
| Ark | ~0.05% (max(1, ceil(amount Ã— 0.0005))) |
| On-chain | ~250 sats (1 vByte Ã— 250 sat/vB estimate) |
...
Community Members
    â”‚
    â”œâ”€ Earn sats: workshops, freelance, merchant sales
    â”œâ”€ Spend sats: ecash at local merchants (0 fees)
    â”‚
    â”œâ”€ Agent Revenue: L402 services earn sats 24/7
    â”‚     â”œâ”€ Privacy audits: 500 sats/request
    â”‚     â”œâ”€ Cycle signals: 200 sats/request
    â”‚     â”œâ”€ Data market: 1,000 sats/request
    â”‚     â””â”€ Compute: 2,000 sats/request
    â”‚
    â””â”€ Mint Fee: 0.2% on ecash operations
          â””â”€ Funds community infrastructure
```
...
**Zero merchant fees:** Ecash payments at merchants carry no processing fee â€” a significant advantage over the 2-3% charged by credit card processors.
...
|---|---|---|
| Uptime (30-day) | 25% | â‰¥ 99.5% |
| Payment success rate | 25% | â‰¥ 98% |
| Spend velocity (tx/user/month) | 20% | â‰¥ 5 |
| Monthly active users | 20% | â‰¥ 300 |
| Active merchants | 10% | â‰¥ 30 |
...
|---|---|---|---|
| Nascent | 0â€“10 | < 50 | Early stage, founding merchants |
| Emerging | 10â€“30 | 50â€“200 | Growing adoption, circular patterns forming |
| Growing | 30â€“100 | 200â€“1,000 | Regular circular spending |
...
```
    ...
    â”‚  CDK Mint    â”‚  â”‚   arkd     â”‚  â”‚  SP Indexer      â”‚
    â”‚  (Postgres)  â”‚  â”‚  (VTXOs)   â”‚  â”‚  (BIP-352 scan)  â”‚
    â”‚  Port 3338   â”‚  â”‚  Port 7070 â”‚  â”‚  Port 8100       â”‚
    â””â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”˜  â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜  â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
           â”‚
    â”Œâ”€â”€â”€â”€â”€â”€â”´â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
    â”‚  Fedimint Guardians (3â€“7)  â”‚
    â”‚  Iroh transport            â”‚
    â”‚  v0.10.0 "Lighthouse"      â”‚
    â””â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
           â”‚
    â”Œâ”€â”€â”€â”€â”€â”€â”´â”€â”€â”€â”€â”€â”€â”€â”
    â”‚   Aperture   â”‚    L402 reverse proxy
    â”‚   Port 8081  â”‚    Routes: /api/agent, /api/cycle, /api/privacy
    â””â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”˜
           â”‚
    â”Œâ”€â”€â”€â”€â”€â”€â”´â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”     â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
    â”‚    Prometheus      â”‚â”€â”€â”€â”€â†’â”‚   Grafana    â”‚
    â”‚    15s scrape      â”‚     â”‚   Port 3001  â”‚
    â”‚    30-day retain   â”‚     â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
...
- CDK (`cashubtc/cdk-mintd`) for >30 members or mainnet â€” Postgres-backed, Prometheus metrics, Kubernetes-ready
- Nutshell (`cashubtc/nutshell`) for â‰¤30 members on testnet â€” lightweight, minimal infrastructure

**Fedimint**: One `fedimintd` container per guardian. Iroh peer-to-peer transport with DNS-based discovery. No direct Bitcoin RPC â€” relies on the federation's LND gateway for chain data.
...
- Consensus: `ceil(guardianCount Ã— 2/3)`
...
| Privilege escalation | Agent attempts admin operations | Three-tier enforcement: watch-only â†’ pay-only â†’ admin, checked per operation |
...
Tier is set at connection time and cannot be upgraded at runtime. The `pay-only` tier requires a `RemoteSignerConfig` pointing to a separate `litd` process â€” the agent process never holds signing keys.
...
ArxMint's first production deployment targets the Boulder-Longmont corridor in Colorado â€” a community with 70+ active Bitcoiners, regular meetups, and an existing Signal group discussing circular economy development.
...
- Recruit 3â€“5 founding merchants
...
**Merchant Expansion (Months 2â€“3)**
...
- BCE maturity tier: Emerging â†’ Growing
...
**Evaluation and Scale (Months 7â€“9)**
...
**Commerce**: 3â€“5 founding merchants recruited, POS hardware set up (QR terminals + Numo NFC cards), end-to-end payment tested.
...
City A User                                    City B User
    â”‚                                              â”‚
    â””â”€ Melt ecash â†’ Lightning invoice â”€â”€â”€â”€â”€â”€â”€â”€â†’ Mint ecash
                    (~10 sats fee)
                    (~3 seconds)
```
...
### Phase 1: Keystone â€” Core Architecture
...
### Phase 2: Spire â€” Full Privacy and Commerce Stack
...
### Phase 4: Citadel â€” Production Deployment
...
**The solution:** Merchants run their own ArxMint payment node. Customers pay the merchant's node directly â€” peer-to-peer via ecash or Lightning. Settlement is instant (no T+2 delay). Chargebacks are impossible (bearer ecash is final). No payment data leaves the merchant's infrastructure.
...
```
Phase 0 (Security) â”€â”€â†’ Phase 1 (Core)
    Phase 1.1 (NUT-24) â”€â”€â†’ Phase 2.8 (Gateway Bridge)
    Phase 1.2 (Spend Router) â”€â”€â†’ Phase 2.2 (Ark SDK)
    Phase 1.3 (BCE Metrics) â”€â”€â†’ Phase 4.1 (Pilot)
    Phase 1.4 (Merchant Flow) â”€â”€â†’ Phase 4.1 (Pilot)
Phase 2 (Privacy) â”€â”€â†’ Phase 3 (Advanced)
    Phase 2.4 (Coco Multi-Mint) â”€â”€â†’ Phase 4.5 (Multi-City)
...
Phase 5 depends on: Phase 4 (pilot validation) + Phase A-E (production infrastructure)
    Phase 5.8d (Stack) â”€â”€â†’ 5.8a (Provisioning) â”€â”€â†’ 5.8b (DNS/Connectivity) â”€â”€â†’ 5.8c (LSP Bootstrap)
    Phase 5.8a â”€â”€â†’ 5.1 (Local Auth) + 5.2 (Webhooks) + 5.3 (Checkout) + 5.4 (Status API) + 5.5 (Client SDK) + 5.6 (LNURL) + 5.7 (Dashboard)
    Phase 5.8a â”€â”€â†’ 5.11a (Updates) â”€â”€â†’ 5.11b (Backups) â”€â”€â†’ 5.11c (Restore)
```
...
**Bitcoin Indonesia / Fedi**: Demonstrated Fedimint federation deployment at scale (10â€“20K users). Validates the federated custody model for emerging markets. ArxMint's Community Generator draws directly from this deployment pattern.
...
Bitcoin's promise was peer-to-peer electronic cash. Fifteen years later, most Bitcoin is held, not spent. The infrastructure gap is real: no local mints, no private payment rails, no merchant directories, no way for AI agents to participate in the economy. And the existing payment infrastructure â€” Stripe, Square, PayPal â€” extracts 2.9%+ from every transaction while selling customer data.
...
The thesis is simple: **humans and AI agents can share the same private commerce infrastructure â€” and merchants can accept payments on it with app-like simplicity while keeping self-custody.** The same sats that pay for coffee at a local shop can pay an AI agent for a privacy audit. The same mint that issues ecash to a community member issues ecash to an autonomous process. The same non-custodial merchant node powers both human checkout and programmatic settlement between federated marketplaces.

The Boulder-Longmont pilot will be the first test. Thirty merchants. Three hundred users. Six months. If the model works â€” and the technology says it should â€” the replication playbook and merchant platform make it available to every community and every merchant in the world.
...
| Term | Definition |
|---|---|
| **BCE** | Bitcoin Circular Economy â€” a community where bitcoin circulates as money |
| **Cashu** | Lightweight ecash protocol with single-operator mint |
| **CDK** | Cashu Development Kit â€” cloud-native Cashu mint implementation |
...
| **DKG** | Distributed Key Generation â€” ceremony where Fedimint guardians jointly create signing keys |
| **Ecash** | Electronic cash â€” bearer tokens that can be transferred without identity |
| **FBCE** | Free Bitcoin Circular Economies â€” grant program funding BCEs globally |
...
| **MCP** | Model Context Protocol â€” standard interface connecting AI agents to tools |
...
| **P2BK** | Pay-to-Blinded-Key â€” conditional ecash transfer requiring recipient's key |
...
| **VTXO** | Virtual UTXO â€” off-chain Bitcoin output in the Ark protocol |

## Appendix B: Research Traceability

Every architectural decision in ArxMint traces to published research and protocol specifications. The full cross-reference mapping 7 research documents to 25 roadmap items and 36 identified gaps is maintained in `docs/research/research-crossref.md`.
