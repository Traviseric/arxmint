# ArxMint â€” Overnight Tasks

**Goal:** Ship merchant MVP from the current prototypes â€” a merchant answers 3 questions, runs one command, and accepts Bitcoin payments in 15 minutes.
**Spec:** `docs/core/spec.md` Â§4.1, Â§5 Flow E, Â§6.2, Â§10 (acceptance criteria for every item below)
**Roadmap:** `docs/core/roadmap.md` Phase 5 (Bazaar)
...
**Rule for overnight workers:** Do not rebuild these surfaces from scratch. Promote the existing prototypes into the self-hosted architecture defined in `docs/core/roadmap.md`.

---

## Task List (ordered, top to bottom)

### Cleanup â€” Fix carry-forward bugs before building new features
...
- [ ] **T01** Wire `privacy-audit` endpoint to real `computePrivacyScore()`
  ...
  - Spec: Â§10 item 1.1 acceptance criteria
  - Test: Unit test â€” score varies by community config input
...
- [ ] **T02** Persist L402 challenges to DB
  ...
  - Test: Create challenge â†’ restart server â†’ challenge still retrievable
...
- [ ] **T04** Fix L402 auth bypass + hardcoded secrets
  ...
  - (c) Remove any hardcoded session secret fallback â€” fail if env var missing
...
- [ ] **T05** Fix merchant false-success on DB failure
  ...
  - If merchant DB write fails, show error â€” not success. The active `/create` wizard must surface real generation/deploy errors with retry; do not spend time on the legacy `components/create-community-form.tsx` unless deleting or retiring it.
  - Test: Mock DB failure â†’ UI shows error state with retry option on the active flow

- [ ] **T06** Add CI/CD pipeline
  - File: `.github/workflows/ci.yml`
  - GitHub Actions: lint â†’ build â†’ `npm test` â†’ E2E tests (regtest Docker) on every push to master. Deploy to testnet on tagged release.
  - Test: Push to master triggers workflow. All tests pass in CI.

- [ ] **T07** Landing page cleanup
  - File: `app/page.tsx`
  - (a) Old `Math.random()` live metrics removed in homepage redesign â€” verify no fake counters remain
...
### Phase 5 MVP â€” Merchant payment node

#### 5.1 â€” Local Auth Tokens
...
- [ ] **T08** Implement scoped merchant macaroons
  ...
  - `arx_pub_` CANNOT authorize payments or read balances â€” safe for client-side embedding
  ...
  - Spec: Â§10 item 5.1 â€” permission tests, rotation tests, sandbox routing
  - Test: `arx_pub_` blocked from pay/balance endpoints. Old token rejected after rotation.

#### 5.2 â€” Webhook Engine
...
- [ ] **T09** Build webhook engine
  ...
  - Spec: Â§10 item 5.2 â€” end-to-end test: create invoice â†’ pay â†’ webhook fires with valid signature
  - Test: Block endpoint â†’ verify 5 retries with backoff. Same event not delivered twice (idempotency).

#### 5.3 â€” Self-Hosted Checkout
...
- [ ] **T10** Promote checkout prototype to self-hosted checkout
  ...
  - Spec: Â§10 item 5.3 â€” verify invoice pubkey matches merchant node identity
  - Test: Full payment flow from external wallet (Phoenix/Zeus/Breez). Settlement updates in real time.

#### 5.4 â€” Payment Status API
...
- [ ] **T11** Upgrade prototype status polling into merchant payment status APIs
  ...
  - Target state: deterministic states (`pending` â†’ `completed` | `expired` | `failed`), cursor-paginated history, merchant-local API shape, SSE `status_changed` stream, and no backwards state transitions.
  - Spec: Â§10 item 5.4 â€” API integration test through all states
  - Test: Create payment â†’ progress through states â†’ verify payload. Unknown IDs return proper error.
...
- [ ] **T12** Build `@arxmint/js` client SDK
  ...
  - < 15KB bundled
  - Spec: Â§10 item 5.5 â€” no requests to arxmint.com during payment flow
  - Test: SDK â†’ merchant node â†’ payment â†’ callback. Teneo stub interface compatibility.

- [ ] **T13** Build `@arxmint/react` components
  - Files: new `packages/react/` directory (or `lib/sdk/react/`)
  - `<PayButton>` â€” renders pay button, opens checkout on click
  - `<CheckoutForm>` â€” inline checkout with QR + status
  - `<QRPayment>` â€” standalone QR display with SSE status
...
- [ ] **T14** Implement LNURL-pay endpoints
  ...
  - Spec: Â§10 item 5.6 â€” LNURL metadata validation (spec compliance)
  - Test: Payable from 3+ external LNURL-compatible wallets. Static QR scan-and-pay.

#### 5.7 â€” Merchant Dashboard
...
- [ ] **T15** Build merchant dashboard
  ...
  - Push notifications for wallet unlock (non-custodial â€” secret goes to node, never ArxMint)
  - Spec: Â§10 item 5.7 â€” health indicators reflect actual node state
  - Test: Mock degraded LND â†’ dashboard shows yellow/red. CSV export matches displayed data. Dashboard requires admin macaroon.
...
- [ ] **T16** Finish merchant init wizard from beta UX to real deploy path
  ...
  - Target flow: "Where to run?" (cloud/home/advanced) â†’ "Store name?" â†’ "Online or in-person?" â†’ generates merchant-specific stack or calls provisioning path.
  ...
  - Spec: Â§10 item 5.8 â€” running node with managed subdomain + HTTPS in < 15 min
...
- [ ] **T17** LSP liquidity bootstrap
  ...
  - Spec: Â§10 item 5.8c â€” regtest test with mock LSP
  - Test: Create merchant node â†’ send inbound payment â†’ verify channel opened and payment received.
...
- [ ] **T18** Managed DNS + connectivity
  ...
  - Spec: Â§4.1 connectivity model + managed DNS section
  - Test: DNS resolves to node. Tunnel establishes. Checkout reachable from public internet.

---

### Post-MVP â€” Ship after merchant flow works end-to-end

- [ ] **T19** Idempotency + production hardening (Spec Â§10 item 5.10)
  - `Idempotency-Key` header on all payment endpoints. Structured error codes. Rate limiting on merchant API.

- [ ] **T20** Stack update engine (Spec Â§10 item 5.11a)
  - Signed stack BOMs. Maintenance window application. Auto-rollback on failed health check. Canary rings.

- [ ] **T21** Zero-knowledge encrypted backups (Spec Â§10 item 5.11b)
  - LND SCB event-driven. Cashu DB encrypted with seed-derived key. Automated restore rehearsal.

- [ ] **T22** One-click restore (Spec Â§10 item 5.11c)
  - Fresh host â†’ seed phrase â†’ derive key â†’ fetch encrypted backup â†’ restore. Post-restore health checks.
