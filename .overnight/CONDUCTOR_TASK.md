...
## Lessons from Previous Runs (Known False Positives)
  - overnight_tasks: "OVERNIGHT_TASKS.md ID 16 — Write DEPLOY.md" — Already done. DEPLOY.md exists at project root.
  - overnight_tasks: "OVERNIGHT_TASKS.md ID 17 — Add CDK mint option alongside Nutshell" — Already done. docker/docker-compose.cdk.yml exists.
  - feature_audit: "Agent marketplace: replace compute stub with real job queue, replace static datasets with live mempool/LN API calls" — High effort, requires DB + auth + LND infrastructure to be in place first. Not a single-task fix. Requires product decision on real agent backends.
  - feature_audit: "Cycle monitoring: integrate Glassnode or on-chain data API for real UTXO-based MVRV/NUPL" — Requires paid external API or significant on-chain data pipeline. Business/infrastructure decision, not a code fix. cycle-monitor.ts already acknowledges this limitation in a code comment.
  - code_quality_audit: "lib/nostr-auth.ts:186 — verifyNip98Auth() catch swallows JSON parse/atob errors" — Intentional security behavior — no sensitive info should leak. Audit itself labels it 'acceptable' and 'intentional'. Low priority.
  - code_quality_audit: "lib/store.ts:155 — hydrateCashuSession/hydrateNostrSession empty catch blocks" — Intentional graceful degradation for localStorage failures. Not worth a worker task.
  - code_quality_audit: "components/merchant-onboard.tsx:588 — NFC provisioning TODO (navigator.nfc.write)" — Requires physical Numo NFC hardware. Business decision. Already logged in HUMAN_TASKS.md (HT-004).
  - code_quality_audit: "lib/cashu-sdk.ts:1553 — NUT-XX Programmable eCash dead type definitions" — Aspirational types. Moving to types/ file is low value — clearly marked as unimplemented. No runtime impact.
  - code_quality_audit: "lib/lightning-agent.ts:286 — console.log in production payInvoice()" — Minor debug log. Not blocking functionality. Not worth a task.
  - code_quality_audit: "app/api/l402/route.ts:14 — pendingL402 Map unbounded (no max size, 24h TTL)" — Minor. App works correctly for normal usage. Over-engineering for current stage.
