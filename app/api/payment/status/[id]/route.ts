...
export async function GET(
  ...
  return observeApiRoute(request, "/api/payment/status/[id]", async () => {
    ...
    if (!entry) {
      try {
        ...
        if (row && row.notes) {
          const data = JSON.parse(row.notes) as {
    ...
    // Minimal safe response for unauthenticated callers (e.g. polling for payment status).
